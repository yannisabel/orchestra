...
const getGlobalRegistry = (): IconLibrary[] => {
  if (typeof window !== 'undefined') {
    if (!(window as any)[ICON_REGISTRY_KEY]) {
      (window as any)[ICON_REGISTRY_KEY] = [defaultLibrary, orchestraLibrary];
    }
    return (window as any)[ICON_REGISTRY_KEY];
  }
  // Fallback for non-browser environments
  if (!(globalThis as any)[ICON_REGISTRY_KEY]) {
    (globalThis as any)[ICON_REGISTRY_KEY] = [defaultLibrary, orchestraLibrary];
  }
 ...
 */
export const registerIconLibrary = (name: string, options: Except<IconLibrary, 'name'>): void => {
