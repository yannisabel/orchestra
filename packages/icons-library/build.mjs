...
files.forEach(file => {
  ...
  if (originalContent.includes("export { ") && originalContent.includes(" from ")) {
    const match = originalContent.match(/export\s*{\s*([^}]+)\s*}\s*from\s*['"]([^'"]+)['"]/g)
    if (match) {
      const importMatch = originalContent.match(/export\s*{\s*([^}]+)\s*}\s*from\s*['"]([^'"]+)['"]/)
      if (importMatch) {
